-- =====================================================
-- SUPPORTLY - COMPLETE DATABASE RESET AND SETUP
-- =====================================================
-- This script safely resets and recreates the entire database
-- Safe to run multiple times - it will clean up everything first
-- =====================================================
...
DROP POLICY IF EXISTS "NFT configs are viewable by everyone" ON nft_configs;
DROP POLICY IF EXISTS "Creators can manage own NFT configs" ON nft_configs;
DROP POLICY IF EXISTS "Content is viewable based on gating" ON content;

-- Drop all triggers
DROP TRIGGER IF EXISTS update_creators_updated_at ON creators;
DROP TRIGGER IF EXISTS update_nft_configs_updated_at ON nft_configs;
...
SELECT 'Supportly database reset and setup completed successfully! 🎉' as status;
